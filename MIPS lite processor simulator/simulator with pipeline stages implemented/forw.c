...
#define Debug 2 //0 - Results, 1 - timing simulaotr, 2 - functional simulator, 4 - all
...
struct s hazard(){
  switch(s[1].op)
  {
    case 0xD: if(s[0].op == 0xD) s[1] = s[0]; //RAW hazards
              if(Debug == 1) printf("\nHZ R[%d]  mem[%d] %x + %x\tLDW", s[0].rt, s[0].rs, mem[R[s[0].rs]+s[0].imm], s[0].imm);

    //Control flow instructions
    case 0xC: R[s[0].rt] = mem[R[s[0].rs] + s[0].imm];//LDW s[3].rt s[3].rs Imm
              if(Debug == 1) printf("\nHZ R[%d]  mem[%d] %x + %x\tLDW", s[0].rt, s[0].rs, mem[R[s[0].rs]+s[0].imm], s[0].imm);
              if(!drain) mem_i++;
              break;
              //WAR
    case 0x10: if(s[0].op == 0xD) s[1] = s[0]; //RAW hazards
              if(Debug == 1) printf("\nHZ R[%d]  mem[%d] %x + %x\tLDW", s[0].rt, s[0].rs, mem[R[s[0].rs]+s[0].imm], s[0].imm);

    //Control flow instructions
    case 0xF: R[s[0].rt] = mem[R[s[0].rs] + s[0].imm];//LDW s[3].rt s[3].rs Imm
              if(Debug == 1) printf("\nHZ R[%d]  mem[%d] %x + %x\tLDW", s[0].rt, s[0].rs, mem[R[s[0].rs]+s[0].imm], s[0].imm);
              if(!drain) mem_i++;
              break;
    case 0x3f: s[0].i = 0xffffffff;
  }
  return s[0];
}
